 ...
 */
class PlugandPay_WC_TBC_Checkout_Gateway extends WC_Payment_Gateway {
	...
	public function __construct( $software_version ) {
		...
		$this->id                 = 'tbc_checkout_gateway';
		$this->has_fields         = false;
		$this->method_title       = __( 'TBC (Checkout)', 'tbc-gateway' );
		$this->method_description = __( 'Accept Visa/Mastercard payments in your WooCommerce shop using TBC (Checkout) gateway.', 'tbc-gateway' );
		$this->supports           = [
			'products',
			'refunds',
		];
	...
	public function init() {
		$this->form_fields = include 'settings/checkout-gateway.php';
		$this->init_settings();
	...
	public static function log( $message, $level = 'info' ) {
		if ( self::$log_enabled ) {
			...
			self::$log->log( $level, $message, [ 'source' => 'tbc_checkout_gateway' ] );
		}
	}
	...
	public function admin_notices() {
		...
		if ( ! $this->has_required_options() ) {
			...
			echo '<div class="error"><p>' . wp_kses_data( sprintf( __( 'WooCommerce TBC Credit Card Payment Gateway (Checkout): Please fill out required options <a href="%s">here</a>.', 'tbc-gateway' ), admin_url( 'admin.php?page=wc-settings&tab=checkout&section=' . $this->id ) ) ) . '</p></div>';
		}
	}
			...
			<?php if ( self::$log_enabled && $this->client_accounts ) { ?>
				<?php foreach ( $this->client_accounts as $account ) { ?>
					<p style="color:green;">
						...
						<?php echo $this->get_access_token( $account['currency'] ) ? esc_html( __( 'Access token retrieved. Expires: ', 'tbc-gateway' ) . date_i18n( get_option( 'date_format' ), get_option( $transient_key ) ) . ', For currency: ' . $account['currency'] ) : ''; ?>
					</p>
				<?php } ?>
			<?php } ?>
		</div>
	...
	public function process_payment( $order_id ) {
		...
		if ( ! $response || ! isset( $response['payId'], $response['links'], $response['status'] ) || 'Created' !== $response['status'] ) {
			...
			wc_add_notice( __( 'TBC Checkout not responding, please try again later.', 'tbc-gateway' ), 'error' );
			return;
		}
	...
	public function route_return() {
			...
			if ( $response && isset( $response['status'] ) ) {
				...
				switch ( $response['status'] ) {
					...
					case 'Failed':
						$this->payment_failed( $order, $trans_id );
						$this->log( 'TBC Checkout: payment failed.', 'notice' );
						wc_add_notice( __( 'Your payment was declined', 'tbc-gateway' ), 'error' );
		...
		wc_add_notice( __( 'Something went wrong.', 'tbc-gateway' ), 'error' );
		wp_safe_redirect( wc_get_page_permalink( 'cart' ) );
		exit;
	}
	...
	public function payment_complete( $order, $transaction_id ) {
		if ( $order->payment_complete() ) {
			...
			$order->add_order_note( sprintf( __( 'TBC Checkout payment complete (transaction_id: %s)', 'tbc-gateway' ), $transaction_id ) );
			return true;
		}
		return false;
	}
	...
	public function payment_failed( $order, $transaction_id ) {
		/* translators: Transaction ID */
		$order_note = sprintf( __( 'TBC Checkout payment failed (transaction_id: %s)', 'tbc-gateway' ), $transaction_id );
	...
	public function generate_client_accounts_html() {
		...
		?>
		<tr valign="top">
			<th scope="row" class="titledesc"><?php esc_html_e( 'Client accounts:', 'tbc-gateway' ); ?></th>
				...
				<div class="wc_input_table_wrapper">
					<table class="widefat wc_input_table sortable" cellspacing="0">
						<thead>
							<tr>
								<th class="sort">&nbsp;</th>
								<th><?php esc_html_e( 'Currency', 'tbc-gateway' ); ?></th>
								<th><?php esc_html_e( 'Client Id', 'tbc-gateway' ); ?></th>
								<th><?php esc_html_e( 'Client Secret', 'tbc-gateway' ); ?></th>
							</tr>
						</thead>
