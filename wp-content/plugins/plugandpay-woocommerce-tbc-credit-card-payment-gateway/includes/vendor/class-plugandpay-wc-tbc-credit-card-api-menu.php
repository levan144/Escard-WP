...
if ( ! class_exists( 'PlugandPay_WC_TBC_Credit_Card_API_Menu' ) ) {
	class PlugandPay_WC_TBC_Credit_Card_API_Menu {
		...
		public function update_check( $transient ) {
			...
			if ( isset( $license_status[ 'activated' ] ) && $license_status[ 'activated' ] === 'inactive' ) {
				update_option( $this->ame_activated_key, 'Deactivated' );
			}

			if ( ! $license_status && get_option( $this->ame_activated_key ) === 'Activated' ) {
				$opt     = sprintf( '%s_offline_%s', $this->data_prefix, date( 'm_Y' ) );
