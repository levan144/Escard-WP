...
?>
<div class="wrap">
	<h2><?php echo wp_kses_post( $default_option['settings']['your_profile_confirm_heading'] === $this->option['settings']['your_profile_confirm_heading'] ? __( 'Delete Account', 'delete-me' ) : $this->option['settings']['your_profile_confirm_heading'] ); ?></h2>
	<form action="<?php echo esc_url( add_query_arg( array( $this->info['trigger'] => $this->user_ID, $this->info['nonce'] => wp_create_nonce( $this->info['nonce'] ) ) ) ); ?>" method="post">
		...
		<table class="form-table">
			<tr class="form-field<?php if ( !empty( $this->POST[$this->info['trigger'] . '_your_profile_confirm_password'] ) ) echo ' form-invalid'; ?>">
				<td style="padding: 0;">
					<label for="<?php echo $this->info['trigger']; ?>_your_profile_confirm_password"><?php echo wp_kses_post( $default_option['settings']['your_profile_confirm_password_label'] === $this->option['settings']['your_profile_confirm_password_label'] ? __( 'Password', 'delete-me' ) : $this->option['settings']['your_profile_confirm_password_label'] ); ?></label> <input style="width: 15em;" type="password" autocomplete="off" autofocus id="<?php echo $this->info['trigger']; ?>_your_profile_confirm_password" name="<?php echo $this->info['trigger']; ?>_your_profile_confirm_password" />
				</td>
			</tr>
		</table>
		<?php endif; ?>
