...
function custom_add_to_cart_and_redirect() {
    if (isset($_GET['add_product_to_cart']) && isset($_GET['product_id'])) {
        $product_id = $_GET['product_id']; // Replace this with your product ID
		WC()->cart->empty_cart();
        ...
        // If the user is logged in, redirect to cart or another desired page.
        
    }
}
add_action('template_redirect', 'custom_add_to_cart_and_redirect');

function disable_nickname_field($fields) {
    if (isset($fields['nickname'])) {
        $fields['nickname']['input'] = 'hidden';
    }
    return $fields;
}
add_filter('woocommerce_customer_meta_fields', 'disable_nickname_field');
